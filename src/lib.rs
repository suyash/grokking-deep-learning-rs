...
pub fn elementwise_multiplication(vec_a: Vector, vec_b: &[f64]) -> Vector {
    vec_a
        .into_iter()
        .zip(vec_b.iter())
        .map(|(a, b)| a * b)
...
pub fn dot(vec_a: Vector, vec_b: &[f64]) -> f64 {
    elementwise_multiplication(vec_a, vec_b).into_iter().sum()
}
